      ...
      <v-card-text>
        <v-row no-gutters>
          ...
          <v-col cols="6" md="6" sm="12" align="center" class="mb-3">
            <v-avatar size="300" v-if="!isEditing" color="grey lighten-2">
              <img alt="Avatar" :src="'Avatar/' + delivery.avatar" />
            </v-avatar>

            <v-hover v-slot="{ hover }" close-delay="100" v-if="isEditing">
              <v-avatar
                size="300"
                @click="rander('image-file')"
                :style="
                  hover ? 'border:2px solid #7367f0' : 'border:2px solid black'
                "
              >
                <img alt="Avatar" :src="'Avatar/' + delivery.avatar" />
    ...
    expected() {
      {
        ...
        if (expected != undefined) {
          if (expected.status === "success") {
            this.$store.commit("FETCH_GUARD", expected.result.subMessage);
